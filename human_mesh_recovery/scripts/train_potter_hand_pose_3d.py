...
def train(config, train_loader, model, criterion, optimizer, epoch, output_dir, device, logger, writer_dict):
    ...
    for i, (input, pose_2d_gt, hm_gt, target_weight, pose_3d_gt, vis_flag, _) in enumerate(train_loader):
        ...
        if (i+1) % config.PRINT_FREQ == 0:
            ...
            writer.add_scalar('train_loss', total_loss.avg, global_steps)
    ...
    with torch.no_grad():
        ...
        for i, (input, pose_2d_gt, hm_gt, target_weight, pose_3d_gt, vis_flag, _) in enumerate(val_loader):
            ...
            if (i+1) % config.PRINT_FREQ == 0:
                ...
                writer.add_scalar('valid_loss_loss', total_loss.avg, global_steps)
...
def main():
    ...
    train_dataset = ego4dDataset(DATA_ROOT_DIR, anno_type='annotation', split='train', transform=transform)
    ...
    for epoch in range(cfg.TRAIN.BEGIN_EPOCH, cfg.TRAIN.END_EPOCH):
        ...
        train(cfg, train_loader, model, criterion, optimizer, epoch, final_output_dir, device, logger, writer_dict)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'val_loss': val_loss,
            }, os.path.join(final_output_dir, f"POTTER-HandPose-{cfg.DATASET.DATASET}.pt"))
